...
def compare_results():
    ...
    for i in range(0,len(gresults)):
        ...
        for j in range(0,len(gresults)):
            #print "  " + str(j)
            if j > i :
                print "compare keys %s vs %s" % (gresults.keys()[i], gresults.keys()[j])
                for k in range(0,len(gresults[gresults.keys()[0]])):
                    ...
                    if (elem1[k] == elem2[k]):
                        ...
                        print "%2d: DO NOT MATCH%s" % (k, empty)
                        print "%s: %s" % (gresults.keys()[i], elem1[k])
                        print "%s: %s" % (gresults.keys()[j], elem2[k])
...
def Init():
    ...
    print "using as test: %s" % test
    print "using as test path: %s" % test_path    
    print "using as init_file: %s" % init_file
...
def main():
    ...
    print "======================================================"
    print "Working on test %s" % testfile
    print "Expecting output from %s" % outfile
    print "======================================================"
    ...
    print json.dumps(gresults,  sort_keys=True,
                     indent=4, separators=(',', ': '))
